...
def test_launcher_resolves_codex_and_inherits_console_streams(monkeypatch):
    ...
    monkeypatch.setattr("lark_bot.codex_tui.shutil.which", lambda value: "C:/tools/codex.cmd")
    monkeypatch.setattr("lark_bot.codex_tui._read_existing_notify", lambda value: None)
...
def test_launcher_forwards_prompt_model_and_sandbox_verbatim(monkeypatch):
    monkeypatch.setattr("lark_bot.codex_tui.shutil.which", lambda value: "/usr/bin/codex")
    monkeypatch.setattr("lark_bot.codex_tui._read_existing_notify", lambda value: None)
    seen: list[list[str]] = []
...
def test_launcher_reports_missing_codex(monkeypatch):
    monkeypatch.setattr("lark_bot.codex_tui.shutil.which", lambda value: None)
