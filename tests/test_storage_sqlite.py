...
def test_sqlite_store_suppresses_duplicate_within_cooldown(tmp_path):
    store = SQLiteNotificationStore(tmp_path / "lack_bot.sqlite3")
...
def test_sqlite_store_allows_duplicate_after_cooldown(tmp_path):
    store = SQLiteNotificationStore(tmp_path / "lack_bot.sqlite3")
    now = datetime(2026, 1, 1, tzinfo=timezone.utc)
